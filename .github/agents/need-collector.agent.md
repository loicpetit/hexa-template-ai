...
3. **Conflict check**: If overlap is found, draft a replacement proposal referencing the existing REQ ID, state the rationale, and request explicit user approval before proceeding.
4. **Write**: Create one file per requirement at `requirements/REQ-XXXX-<short-title>.md` using the template below.
5. **Update traceability**: Add the new REQ row to `traceability.md` with status `Draft`.
